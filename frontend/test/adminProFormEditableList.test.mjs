...
test('可编辑明细业务方只能声明字段和受控宽度，结构和操作由底座固定', () => {
  const source = read('src/components/admin/AdminProFormEditableList/index.tsx');
  const example = read('src/modules/design-system/pages/sections/input/EditableDetailListExamples.tsx');
  ...
  assert.match(source, /width\?:\s*'compact'\s*\|\s*'standard'\s*\|\s*'wide'/);
  assert.match(source, /rules\?:\s*FormListProps\['rules'\]/);
  assert.match(source, /field\.width/);
  assert.match(example, /fields=\{stageFields\}/);
  assert.match(example, /width:\s*'wide'/);
  assert.match(example, /width:\s*'compact'/);
  assert.doesNotMatch(example, /width:\s*['\"]\\?d+px/);
});
