...
import type { ProjectContractAttachment, ProjectContractFormValues, ProjectContractRecord, ProjectFormValues, ProjectPaymentFormValues, ProjectPaymentRecord, ProjectPlanAdjustment, ProjectPlanDeliveryFile, ProjectPlanItemForm, ProjectPlanItemStatus, ProjectPriority, ProjectRecord, ProjectStagePlan, ProjectStatus } from '../modules/project/types';
import dayjs from 'dayjs';
import { requirementReleasePayload, type RequirementReleaseValues } from '../modules/requirement/components/RequirementReleaseFields';
type Row={id:number;name:string;description?:string;product_id:number;product_name:string;requirement_id?:number;requirement_name?:string;requirement_type?:number;owner_id:number;owner_name:string;members?:Array<{id:number;name:string}>;priority:number;status:number;previous_status?:number;is_overdue:number; overdue_days: number;start_date?:string;expected_end_date:string;actual_end_date?:string;suspend_date?:string;suspend_reason?:string;progress_text?:string;risk_text?:string;creator_name?:string;updater_name?:string;created_at?:string;updated_at?:string};
const rowContract=objectContract<Row>(['id','name','product_id','product_name','owner_id','owner_name','priority','status','is_overdue','overdue_days','expected_end_date']);
...
const map=(r:Row):ProjectRecord=>({id:String(r.id),name:r.name,description:r.description||'',productId:String(r.product_id),productName:r.product_name,requirementId:r.requirement_id?String(r.requirement_id):'',requirementName:r.requirement_name||'-',requirementType:r.requirement_type as ProjectRecord['requirementType'],ownerId:String(r.owner_id),ownerName:r.owner_name,members:(r.members||[]).map(m=>({id:String(m.id),name:m.name})),memberIds:(r.members||[]).map(m=>String(m.id)),priority:Number(r.priority) as ProjectPriority,status:Number(r.status) as ProjectStatus,previousStatus:r.previous_status===undefined?undefined:Number(r.previous_status) as ProjectStatus,isOverdue:Boolean(Number(r.is_overdue)),overdueDays:Number(r.overdue_days),startDate:date(r.start_date),expectedEndDate:date(r.expected_end_date),actualEndDate:date(r.actual_end_date),suspendDate:date(r.suspend_date),suspendReason:r.suspend_reason||'',progressText:r.progress_text||'',riskText:r.risk_text||'',creatorName:r.creator_name||'-',updaterName:r.updater_name||'-',createdAt:dt(r.created_at),updatedAt:dt(r.updated_at)});
...
const payload=(v:ProjectFormValues)=>({requirement_release:requirementReleasePayload(v.requirementRelease),name:v.name,product_id:Number(v.productId),requirement_id:Number(v.requirementId),owner_id:Number(v.ownerId),member_ids:(v.memberIds||[]).map(Number),start_date:formatDateInput(v.startDate),expected_end_date:formatDateInput(v.expectedEndDate),description:v.description||null,progress_text:v.progressText||null,risk_text:v.riskText||null});
...
export async function updateProjectPriority(id:string,priority:ProjectPriority){return unwrap<null>(request.put(`/projects/${id}/priority`,{priority}))}
...
export async function deleteProject(id:string,release?:RequirementReleaseValues){return unwrap<null>(request.delete(`/projects/${id}`,{data:{requirement_release:requirementReleasePayload(release)}}))}
...
const requirementOptionContract=arrayContract(objectContract<{id:number;title:string;product_id:number}>(['id','title','product_id']));
export async function getProjectRequirementOptions(params:{availableOnly?:boolean;projectId?:string}={}){const rows=await unwrap<Array<{id:number;title:string;product_id:number}>>(request.get('/projects/requirement-options',{params:{available_only:params.availableOnly?1:undefined,project_id:params.projectId}}),requirementOptionContract);return rows.map(row=>({label:row.title,value:String(row.id),productId:String(row.product_id)}))}
