...
const payload=(v:ProjectFormValues)=>({name:v.name,product_id:Number(v.productId),requirement_id:Number(v.requirementId),owner_id:Number(v.ownerId),member_ids:(v.memberIds||[]).map(Number),start_date:formatDateInput(v.startDate),expected_end_date:formatDateInput(v.expectedEndDate),description:v.description||null,progress_text:v.progressText||null,risk_text:v.riskText||null});
