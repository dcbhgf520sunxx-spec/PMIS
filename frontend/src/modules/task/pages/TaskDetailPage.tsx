...
export function TaskDetailPage() {
 const businessDay = useBusinessDay();
  ...
  }, [params.id, revision, businessDay]);
  ...
  const changeStatus = async (task: TaskRecord, status: number, values: any) => {
    const result = await updateTaskStatus(task.id, status as TaskRecord['status'], status === 2 ? { actual_end_date: date(values.actualEndTime) } : status === 3 ? { suspend_date: date(values.suspendTime), suspend_reason: values.suspendReason } : {});
  ...
  return <TemplateDetailPage
    ...
    statusSection={row ? { items: [{ label: '任务状态', value: renderTaskStatus(row.status), wide: true }, { label: '优先级', value: renderTaskPriority(row.priority), wide: true }, { label: '逾期状态', value: renderTaskOverdue(row.isOverdue, row.overdueDays), wide: true }] } : null}
