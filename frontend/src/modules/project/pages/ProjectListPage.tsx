...
import { ActionBar, AdminInput, AdminRangePicker, AdminSelect, AdminTextAction, CompactFilterBar, createListFilterItems, DeleteConfirmAction, DetailLinkCell, OperationColumnActions, OverdueTag, PermissionButton, TemplateListPage, useCommittedFilters, useTemplateListPageData, ViewTabs } from '../../../components/admin';
...
export function ProjectListPage() {
  const navigate = useNavigate(); const currentUser = useAuthStore((state) => state.user); const { message } = App.useApp();
  const [view, setView] = useState<'all' | 'mine' | 'joined'>('mine'); const [rows, setRows] = useState<ProjectRecord[]>([]); const [total, setTotal] = useState(0);
  ...
  const filters = useCommittedFilters(defaults); const list = useTemplateListPageData({ rows, total, serverPaging: true, resetOn: [filters.revision, view] });
  ...
  const columns: ProColumns<ProjectRecord>[] = [
    ...
    { title: '项目名称', dataIndex: 'name', width: 180, fixed: 'left', sorter: true, sortOrder: list.sortState.field === 'name' ? list.sortState.order : null, render: (_, row) => <DetailLinkCell onClick={() => navigate(`/projects/${row.id}`)}>{row.name}</DetailLinkCell> },
    ...
    { title: '逾期', dataIndex: 'isOverdue', width: 90, sorter: true, sortOrder: list.sortState.field === 'isOverdue' ? list.sortState.order : null, render: (_, row) => <OverdueTag overdueDays={row.isOverdue ? 1 : 0} /> },
    { title: '预计完成', dataIndex: 'expectedEndDate', width: 120, sorter: true, sortOrder: list.sortState.field === 'expectedEndDate' ? list.sortState.order : null }, { title: '项目成员', dataIndex: 'members', width: 180, ellipsis: true, sorter: true, sortOrder: list.sortState.field === 'members' ? list.sortState.order : null, render: (_, row) => row.members.map((member) => member.name).join('、') || '-' },
  ...
  const items = createListFilterItems([
    ...
    { key: 'overdue', label: '是否逾期', node: <AdminSelect size="small" value={filters.draftFilters.isOverdue} options={[{ label: '已逾期', value: 1 }, { label: '未逾期', value: 0 }]} onChange={(value) => filters.setDraftFilters((prev) => ({ ...prev, isOverdue: value }))} /> },
    ...
    { key: 'expectedEndDate', label: '预计完成时间', wide: true, node: <AdminRangePicker size="small" value={filters.draftFilters.expectedEndDateRange as never} placeholder={['开始日期', '结束日期']} onChange={(value) => filters.setDraftFilters((prev) => ({ ...prev, expectedEndDateRange: value || [] }))} /> }
  ]);
  return <TemplateListPage<ProjectRecord> title="项目管理" titleExtra={<ViewTabs value={view} onChange={setView} items={[{ label: '全部项目', value: 'all', count: viewCounts.all }, { label: '我负责的', value: 'mine', count: viewCounts.mine }, { label: '我参与的', value: 'joined', count: viewCounts.joined }]} />} actions={<ActionBar><PermissionButton permission="project" type="primary" onClick={() => navigate('/projects/new')}>新增项目</PermissionButton></ActionBar>} filter={<CompactFilterBar visibleCount={4} items={items} onSearch={filters.commitFilters} onReset={filters.resetFilters} />} table={{ columns, dataSource: list.pagedRows, pagination: false, search: false, onChange: list.handleTableChange, tableAlertRender: false, scroll: { x: 1450 } }} pagination={list.pagination} />;
}
