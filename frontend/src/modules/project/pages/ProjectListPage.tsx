...
import { ActionBar, AdminInput, AdminRangePicker, AdminSelect, AdminTextAction, CompactFilterBar, createListFilterItems, DeleteConfirmAction, DetailLinkCell, OperationColumnActions, PermissionButton, PriorityChangeAction, TemplateListPage, useCommittedFilters, useTemplateServerListData, ViewTabs , listRouteCodecs, useListViewState, usePageReturnNavigation } from '../../../components/admin';
import { deleteProject, getProjectList, getProjectRequirementOptions, updateProjectPriority, updateProjectStatus } from '../../../api/projectApi';
...
export function ProjectListPage() {
  ...
  const [products, setProducts] = useState<Array<{ label: string; value: string }>>([]); const [requirements, setRequirements] = useState<Array<{ label: string; value: string; productId: string }>>([]); const [users, setUsers] = useState<Array<{ label: string; value: string }>>([]);
  const filters = useCommittedFilters(defaults, { urlSync: true, codecs: { name: listRouteCodecs.string, productId: listRouteCodecs.string, requirementId: listRouteCodecs.string, ownerId: listRouteCodecs.string, memberIds: listRouteCodecs.stringArray, priority: listRouteCodecs.number, status: listRouteCodecs.number, isOverdue: listRouteCodecs.number, expectedEndDateRange: listRouteCodecs.dateArray, creatorId: listRouteCodecs.string, createdAtRange: listRouteCodecs.dateArray } });
  const list = useTemplateServerListData<ProjectRecord, { viewCounts: { all: number; mine: number; joined: number } }>({
    ...
    request: async ({ current, pageSize, sortField, sortOrder }) => {
      const expectedRange = filters.appliedFilters.expectedEndDateRange || [], createdAtRange = filters.appliedFilters.createdAtRange || [];
      const result = await getProjectList({ name: filters.appliedFilters.name || undefined, product_id: filters.appliedFilters.productId, requirement_id: filters.appliedFilters.requirementId, filter_owner_id: filters.appliedFilters.ownerId, owner_id: view === 'mine' ? currentUser?.id : filters.appliedFilters.ownerId, joined_user_id: view === 'joined' ? currentUser?.id : undefined, current_user_id: currentUser?.id, member_ids: filters.appliedFilters.memberIds.join(',') || undefined, priority: filters.appliedFilters.priority, status: filters.appliedFilters.status, is_overdue: filters.appliedFilters.isOverdue, expected_end_date_from: dateValue(expectedRange[0]), expected_end_date_to: dateValue(expectedRange[1]), creator_id: filters.appliedFilters.creatorId, created_at_from: dateValue(createdAtRange[0]), created_at_to: dateValue(createdAtRange[1]), page: current, pageSize, sort_field: sortField, sort_order: sortOrder });
  ...
  useEffect(() => { getProductOptions().then((items) => setProducts(items.filter((item) => item.status === 1))); getProjectRequirementOptions().then(setRequirements); getUserOptions().then(setUsers); }, []);
  const columns: ProColumns<ProjectRecord>[] = [
    { title: '序号', width: 60, fixed: 'left', render: (_, __, index) => list.renderIndex(index) },
    { title: '项目名称', dataIndex: 'name', width: 230, fixed: 'left', sorter: true, sortOrder: list.sortState.field === 'name' ? list.sortState.order : null, render: (_, row) => <div className="project-name-cell"><DetailLinkCell className="project-name-cell__text" title={row.name} onClick={() => navigate(`/projects/${row.id}`)}>{row.name}</DetailLinkCell>{row.isOverdue ? <span className="project-name-cell__tag">{renderProjectOverdue(true, row.overdueDays)}</span> : null}</div> },
    { title: '所属产品', dataIndex: 'productName', width: 150, sorter: true, sortOrder: list.sortState.field === 'productName' ? list.sortState.order : null },
    { title: '所属需求', dataIndex: 'requirementName', width: 180, sorter: true, sortOrder: list.sortState.field === 'requirementName' ? list.sortState.order : null },
    { title: '负责人', dataIndex: 'ownerName', width: 120, sorter: true, sortOrder: list.sortState.field === 'ownerName' ? list.sortState.order : null },
    { title: '优先级', dataIndex: 'priority', width: 90, sorter: true, sortOrder: list.sortState.field === 'priority' ? list.sortState.order : null, render: (_, row) => renderProjectPriority(row.priority) },
    ...
    { title: '操作', valueType: 'option', width: 190, fixed: 'right', render: (_, row) => <OperationColumnActions><AdminTextAction onClick={() => navigate(`/projects/${row.id}/edit`)}>编辑</AdminTextAction><ProjectStatusChangeAction variant="text" project={row} onConfirm={async (status, values) => { await updateProjectStatus(row.id, status, status === 2 ? { actual_end_date: dateValue(values.actualEndDate) } : status === 3 ? { suspend_date: dateValue(values.suspendDate), suspend_reason: values.suspendReason } : {}); message.success('状态更新成功'); await load(); }}>状态变更</ProjectStatusChangeAction><PriorityChangeAction variant="text" permission="project_priority_adjust" current={row.priority} onConfirm={async (priority) => { await updateProjectPriority(row.id, priority); message.success('优先级调整成功'); await load(); }} /><AdminTextAction onClick={() => setFollowUpTarget({ type: 'project', id: row.id, name: row.name })}>跟进记录</AdminTextAction><DeleteConfirmAction variant="text" entityName="项目" targetName={row.name} successMessage="删除成功" onConfirm={async () => { await deleteProject(row.id); await load(); }}>删除</DeleteConfirmAction></OperationColumnActions> }
  ];
  const items = createListFilterItems([
    ...
    { key: 'product', label: '所属产品', node: <AdminSelect size="small" value={filters.draftFilters.productId} options={products} onChange={(value) => filters.setDraftFilters((prev) => ({ ...prev, productId: value, requirementId: undefined }))} /> },
    { key: 'requirement', label: '所属需求', node: <AdminSelect size="small" value={filters.draftFilters.requirementId} options={requirements.filter((item) => !filters.draftFilters.productId || item.productId === filters.draftFilters.productId)} onChange={(value) => filters.setDraftFilters((prev) => ({ ...prev, requirementId: value }))} /> },
