...
import { deleteProject, getProjectList, getProjectRequirementOptions, updateProjectStatus } from '../../../api/projectApi';
...
export function ProjectListPage() {
  ...
  const filters = useCommittedFilters(defaults, { urlSync: true, codecs: { name: listRouteCodecs.string, productId: listRouteCodecs.string, requirementId: listRouteCodecs.string, ownerId: listRouteCodecs.string, memberIds: listRouteCodecs.stringArray, status: listRouteCodecs.number, isOverdue: listRouteCodecs.number, expectedEndDateRange: listRouteCodecs.dateArray, creatorId: listRouteCodecs.string, createdAtRange: listRouteCodecs.dateArray } });
  const list = useTemplateServerListData<ProjectRecord, { viewCounts: { all: number; mine: number; joined: number } }>({
    ...
    request: async ({ current, pageSize, sortField, sortOrder }) => {
      ...
      const result = await getProjectList({ name: filters.appliedFilters.name || undefined, product_id: filters.appliedFilters.productId, requirement_id: filters.appliedFilters.requirementId, filter_owner_id: filters.appliedFilters.ownerId, owner_id: view === 'mine' ? currentUser?.id : filters.appliedFilters.ownerId, joined_user_id: view === 'joined' ? currentUser?.id : undefined, current_user_id: currentUser?.id, member_ids: filters.appliedFilters.memberIds.join(',') || undefined, status: filters.appliedFilters.status, is_overdue: filters.appliedFilters.isOverdue, expected_end_date_from: dateValue(expectedRange[0]), expected_end_date_to: dateValue(expectedRange[1]), creator_id: filters.appliedFilters.creatorId, created_at_from: dateValue(createdAtRange[0]), created_at_to: dateValue(createdAtRange[1]), page: current, pageSize, sort_field: sortField, sort_order: sortOrder });
