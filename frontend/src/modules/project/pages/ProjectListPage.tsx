...
export function ProjectListPage() {
  ...
  const columns: ProColumns<ProjectRecord>[] = [
    ...
    { title: '操作', valueType: 'option', width: 190, fixed: 'right', render: (_, row) => <OperationColumnActions><AdminTextAction onClick={() => navigate(`/projects/${row.id}/edit`)}>编辑</AdminTextAction><ProjectStatusChangeAction variant="text" project={row} onConfirm={async (status, values) => { await updateProjectStatus(row.id, status, status === 2 ? { actual_end_date: dateValue(values.actualEndDate) } : status === 3 ? { suspend_date: dateValue(values.suspendDate) } : {}); message.success('状态更新成功'); await load(); }}>状态变更</ProjectStatusChangeAction><PriorityChangeAction variant="text" permission="project_priority_adjust" current={row.priority} onConfirm={async (priority) => { await updateProjectPriority(row.id, priority); message.success('优先级调整成功'); await load(); }} /><DeleteConfirmAction variant="text" entityName="项目" targetName={row.name} successMessage="删除成功" onConfirm={async () => { await deleteProject(row.id); await load(); }}>删除</DeleteConfirmAction></OperationColumnActions> }
  ];
