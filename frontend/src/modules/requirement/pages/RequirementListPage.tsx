...
export function RequirementListPage() {
  ...
  const [products,setProducts]=useState<any[]>([]),[users,setUsers]=useState<any[]>([]);
  const filters = useCommittedFilters(defaults, { urlSync: true, codecs: { title: listRouteCodecs.string, requirementType: listRouteCodecs.number, status: listRouteCodecs.number, priority: listRouteCodecs.number, isOverdue: listRouteCodecs.number, productId: listRouteCodecs.string, ownerId: listRouteCodecs.string, submitterName: listRouteCodecs.string, submitDateRange: listRouteCodecs.dateArray, expectedEndDateRange: listRouteCodecs.dateArray, creatorId: listRouteCodecs.string, createdAtRange: listRouteCodecs.dateArray } });
  const list = useTemplateServerListData<RequirementRecord, { viewCounts: { all: number; mine: number } }>({
    ...
    request: async ({ current, pageSize, sortField, sortOrder }) => {
      const submitRange=filters.appliedFilters.submitDateRange||[],expectedRange=filters.appliedFilters.expectedEndDateRange||[],createdAtRange=filters.appliedFilters.createdAtRange||[],owner=resolveListViewFilter(view,filters.appliedFilters.ownerId);
      const result = await getRequirementList({ title:filters.appliedFilters.title||undefined, requirement_type: filters.appliedFilters.requirementType, status:filters.appliedFilters.status, priority:filters.appliedFilters.priority, is_overdue: filters.appliedFilters.isOverdue, product_id:filters.appliedFilters.productId, filter_owner_id:owner.filterValue, owner_id:owner.scopeValue, submitter_name:filters.appliedFilters.submitterName||undefined, submit_date_from:date(submitRange[0]),submit_date_to:date(submitRange[1]),expected_end_date_from:date(expectedRange[0]),expected_end_date_to:date(expectedRange[1]),creator_id:filters.appliedFilters.creatorId,created_at_from:date(createdAtRange[0]),created_at_to:date(createdAtRange[1]), view, page: current, pageSize, sort_field: sortField, sort_order: sortOrder });
  ...
  const submitStatus = async (row: RequirementRecord, status: RequirementStatus, values: any) => {
    await updateRequirementStatus(row.id, status, status === 35 ? { pause_date: date(values.pauseDate), pause_reason: values.pauseReason } : [33, 34].includes(status) ? { actual_end_date: date(values.actualEndDate), completion_status: values.completionStatus } : {});
    message.success('状态更新成功'); await load();
  };
  const openDetail=(row:RequirementRecord)=>{const submitRange=filters.appliedFilters.submitDateRange||[],expectedRange=filters.appliedFilters.expectedEndDateRange||[],createdAtRange=filters.appliedFilters.createdAtRange||[],owner=resolveListViewFilter(view,filters.appliedFilters.ownerId);saveDetailNeighborContext(createDetailNeighborContext({moduleKey:'requirement',routeBase:'/requirements',sourcePath:currentPath,params:{title:filters.appliedFilters.title||undefined,requirement_type:filters.appliedFilters.requirementType,status:filters.appliedFilters.status,priority:filters.appliedFilters.priority,is_overdue:filters.appliedFilters.isOverdue,product_id:filters.appliedFilters.productId,filter_owner_id:owner.filterValue,owner_id:owner.scopeValue,submitter_name:filters.appliedFilters.submitterName||undefined,submit_date_from:date(submitRange[0]),submit_date_to:date(submitRange[1]),expected_end_date_from:date(expectedRange[0]),expected_end_date_to:date(expectedRange[1]),creator_id:filters.appliedFilters.creatorId,created_at_from:date(createdAtRange[0]),created_at_to:date(createdAtRange[1]),view,sort_field:list.sortState.field,sort_order:list.sortState.order}}));navigate(`/requirements/${row.id}`)};
  const columns: ProColumns<RequirementRecord>[] = [
    { title: '序号', width: 60, fixed: 'left', render: (_, __, index) => list.renderIndex(index) },
    { title: '需求标题', dataIndex: 'title', width: 240, fixed: 'left', sorter: true, sortOrder: sortOrder('title'), render: (_, row) => <div className="requirement-title-cell"><DetailLinkCell className="requirement-title-cell__text" title={row.title} onClick={() => openDetail(row)}>{row.title}</DetailLinkCell>{row.isOverdue ? <span className="requirement-title-cell__tag">{renderRequirementOverdue(true, row.overdueDays)}</span> : null}</div> },
  ...
  const items = createListFilterItems([
    ...
    ,{key:'productId',label:'所属产品',node:<AdminSelect size="small" value={filters.draftFilters.productId} options={products} onChange={value=>filters.setDraftFilters(prev=>({...prev,productId:value}))}/>}
