...
`v3` 凭证使用 RSA-OAEP-SHA256 加密，并使用独立 HMAC-SHA256 密钥进行来源签名；凭证同时绑定员工、MCP 客户端、Query/Action 入口、签发时间、两分钟失效时间和本次请求 ID。服务端校验签名、使用范围和有效期后，还会消费一次性请求 ID，重复请求直接拒绝。PMIS 正式环境独占 RSA 私钥；调用平台只保存公钥和 HMAC Secret。
