...
Action MCP 在菜单和按钮权限之外还校验业务维护资格，在 `preview` 和 `execute` 两个阶段分别校验。产品、项目、需求、任务、BUG、运维工单六类主单据的普通 `update/delete`，允许当前负责人或该单据自身的创建人操作；创建人仅从数据库 `creator_id` 读取，不接受调用方指定或修改。仅凭创建人身份不能通过普通编辑修改负责人、指派人或跟进人，也不获得状态变更、指派、优先级、排序、子项或附件操作权限。普通主单据新增不要求已有负责人关系；跟进记录保留原有可见范围及操作权限规则，不新增创建人限制。

其他操作仍按原负责人规则：产品、项目和需求按负责人；任务按多负责人列表；BUG 按当前指派人；工单按当前跟进人；关键事项和交付文件按关键事项负责人；阶段、阶段排序、合同、付款及合同附件按所属项目负责人。子任务新增仍按父任务负责人，不能从父记录创建人继承权限。关键事项排序要求列表中全部事项由当前员工负责。批量或排序包含一条无权操作的数据即整批拒绝。权限不符沿用 `MCP_ACTION_NOT_RESPONSIBLE` 错误码，并说明当前所需资格。查询 MCP 的菜单和数据范围不变。
...
4. 查询遵守当前账号已有查询范围。产品、项目、需求、任务、BUG、运维工单的普通编辑和删除允许本人负责或创建的单据，仍须对应菜单和按钮权限；仅凭创建人身份不得修改负责人、指派人或跟进人。其他操作不因创建人身份而放开，普通主单据新增及跟进记录保持现有规则。
...
8. execute 会重新核验当前维护资格。preview 后负责人变化，不代表创建人的普通编辑和删除必然失权；以服务端实时校验为准。若执行被拒绝，不得继续使用旧确认号，应重新查询并如实说明，不得绕过限制。
...
需求搜索、详情及逾期筛选按上海当天实时计算，不依赖保存时的逾期标记；驳回、已完成、已完成未使用及暂停返回空逾期值，既不计入已逾期，也不计入未逾期。
...
项目、需求、任务、工单、阶段关键事项的当前逾期统一由 `overdueRules` 提供对象计算和 SQL 表达式。查询、筛选、数量和排序不依赖持久化逾期缓存，搜索结果增加 `overdue_days`（上海自然日差，未逾期为 0）。需求终态的 `is_overdue` 保持空值；其他业务沿用 0。工单暂停状态 4 不计逾期，父项目暂停时阶段事项不计当前逾期。无计划日期不计逾期。

写入、外部同步及缓存刷新复用相同规则；缓存刷新使用上海时区，不写虚构的人工编辑历史。期间进入逾期仍结合历史状态推导，不以当前值替代历史事实。运维合同复用自然日差，但保留续签、终止及提醒节点规则。

### 需求转项目与项目解除关联

需求状态 `36` 为“已转项目”，Query 可筛选和统计，普通 `requirement_change_status` 不允许设置或退出。`project_create` 成功后自动结束来源需求。需求详情返回只读 `linked_project_id`、`linked_project_name`。

`project_update` 更换所属需求，或 `project_delete` 删除有关联需求的项目时，必须提供 `requirement_release` 对象，由用户明确选择 `status`；33/34 附带 `actual_end_date`、`completion_status`，35 附带 `pause_date`、`pause_reason`。状态必须属于原需求路径且不能是 36。无关联需求的历史项目删除无需该对象。

继续使用原工具权限、负责人规则、preview → 确认 → 相同参数 execute 链路；预览校验恢复状态并展示中文状态。执行将项目、需求及变更历史放在同一事务，完成后查询项目和涉及需求核实。已转项目不计入需求待办或逾期；状态分布中单列。i8 同步保留已转项目状态，不能覆盖交接结果。
