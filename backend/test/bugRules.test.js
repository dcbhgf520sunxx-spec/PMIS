...
test('已修复和已关闭要求对应处理信息', () => {
  ...
  assert.equal(validateBugStatusChange(1, { resolved_date: '2026-07-13', resolution_id: 1 }), '请选择指派人')
  assert.equal(validateBugStatusChange(1, { resolved_date: '2026-07-13', resolution_id: 1, assignee_id: 8 }, '2026-07-13'), null)
  assert.equal(validateBugStatusChange(1, { resolved_date: '2026-07-14', resolution_id: 1, assignee_id: 8 }, '2026-07-13'), '修复时间不能晚于今天（2026-07-13）')
  ...
  assert.equal(validateBugStatusChange(3, { activation_reason: ' 问题再次复现 ' }), '请选择指派人')
  assert.equal(validateBugStatusChange(3, { activation_reason: ' 问题再次复现 ', assignee_id: 6 }), null)
})

test('BUG 状态变更保留或更新指派人', () => {
  const old = { resolved_date: '2026-07-12', closed_date: '2026-07-13', resolution_id: 9, activation_reason: '旧激活原因', assignee_id: 7 }
  assert.deepEqual(resolveBugStatusFields(old, 3, { activation_reason: ' 问题再次复现 ', assignee_id: 6 }), {
  ...
  assert.deepEqual(resolveBugStatusFields(old, 1, { resolved_date: '2026-07-15', resolution_id: 3, assignee_id: 8 }), {
