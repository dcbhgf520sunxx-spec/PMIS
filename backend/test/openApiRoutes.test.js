...
test('路由层拒绝未知动作和优先级变更并返回同一追踪号',async () => {
  const app=express()
  app.use(express.json())
  ...
  await new Promise((resolve)=>server.once('listening',resolve))
  try {
    for (const operation of ['UNKNOWN','CHANGE_PRIORITY']) {
      const response=await fetch(`http://127.0.0.1:${server.address().port}/api/open/v1/requirements/operate`,{method:'POST',headers:{authorization:'Bearer token','content-type':'application/json'},body:JSON.stringify({operation})})
      const body=await response.json()
