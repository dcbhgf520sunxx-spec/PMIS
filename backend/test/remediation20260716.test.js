...
test('软删除后角色和档案类型编码可复用且工单关联有外键', () => {
  ...
  for (const source of [schema, migration]) {
    ...
    assert.match(source, /FOREIGN KEY \(product_id\)[\s\S]*REFERENCES pms_product\(id\)/)
...
test('工单后端校验固定选项、启用跟进人、暂停和激活状态规则', () => {
  ...
  assert.match(statusRules, /0:\s*\[1,\s*2,\s*4\]/)
  assert.match(statusRules, /1:\s*\[2,\s*4\]/)
  assert.match(statusRules, /2:\s*\[4,\s*5\]/)
  assert.match(statusRules, /4:\s*\[0,\s*1,\s*2\]/)
  assert.match(statusRules, /5:\s*\[2\]/)
  assert.doesNotMatch(statusRules, /3:\s*\[/)
