...
test('需求状态变更写日志前必须读取原逾期状态', () => {
  const source = readFileSync(join(controllersDir, 'requirementController.js'), 'utf8')
  assert.match(
    source,
    /exports\.toggleStatus[\s\S]*?baseSelect\('[^']*r\.is_overdue[^']*'\)/,
    '需求状态变更未读取原 is_overdue，会把未变化的状态误记为“空值 -> 未逾期”'
  )
})
