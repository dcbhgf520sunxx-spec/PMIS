...
const SEARCH_OUTPUT_FIELDS = {
  ...
  project_search: {
    ...
    owner_id: outputField('负责人标识'), owner_name: outputField('负责人姓名'),
    members: { type: 'array', items: { type: 'object', additionalProperties: true }, description: '项目成员及其业务名称' },
    priority: outputField('优先级代码'),
  ...
  task_search: {
    ...
    project_name: outputField('关联项目名称'), requirement_id: outputField('关联需求标识'),
    requirement_name: outputField('关联需求名称'), task_type: outputField('任务类型标识'),
    owners: { type: 'array', items: { type: 'object', additionalProperties: true }, description: '任务负责人标识和姓名' },
    owner_names: outputField('负责人姓名，多个用顿号分隔'), task_type_name: outputField('任务类型名称'), priority: outputField('优先级代码'),
  ...
  bug_search: {
    ...
    status_label: outputField('BUG状态中文名称'), project_id: outputField('关联项目标识'),
    project_name: outputField('关联项目名称'), requirement_id: outputField('关联需求标识'),
    requirement_name: outputField('关联需求名称'), bug_type_id: outputField('BUG类型标识'),
    bug_type_name: outputField('BUG类型名称'), resolution_id: outputField('解决方案标识'),
  ...
  work_order_search: {
    ...
    is_overdue_label: outputField('逾期状态中文名称'), product_id: outputField('所属产品标识'),
    product_name: outputField('所属产品名称'), problem_type: outputField('问题类型标识'),
    problem_type_name: outputField('问题类型名称'), follower_id: outputField('跟进人标识'), follower_name: outputField('跟进人姓名'),
  ...
  stage_plan_search: {
    ...
    requires_delivery_file_label: outputField('是否要求交付文件中文名称'),
  ...
  contract_search: {
    id: outputField('合同标识'), project_id: outputField('项目标识'), project_name: outputField('项目名称'), contract_code: outputField('合同编码'),
    contract_name: outputField('合同名称'), signed_date: outputField('签订日期，YYYY-MM-DD'),
    contract_amount: outputField('合同金额，单位：元'), supplier_id: outputField('供应商标识'), supplier_name: outputField('供应商名称'),
  },
  payment_search: {
    id: outputField('付款记录标识'), project_id: outputField('项目标识'), project_name: outputField('项目名称'),
    stage_id: outputField('付款阶段标识'), stage_name: outputField('付款阶段名称'),
    payment_month: outputField('付款月份，YYYY-MM'), payment_amount: outputField('付款金额，单位：元'),
    handler_id: outputField('经办人标识'), handler_name: outputField('经办人姓名'),
  },
  business_attachment_search: {
  ...
  return {
    ...
    properties: {
      ...
      preview: {
        type: 'object',
        properties: {
          tool: { type: 'string', description: '内部业务操作名称' },
          target: { type: 'object', additionalProperties: true, description: '已校验的业务目标' },
          changes: { type: 'object', additionalProperties: true, description: '用于校验和执行的原始业务参数，可能包含内部标识或代码' },
          displayChanges: {
            type: 'object', additionalProperties: true,
            description: '确认界面必须优先展示的业务名称和中文标签；不得用 changes 中的内部标识替代',
          },
        },
        additionalProperties: true,
        description: '待执行操作的可读预览；向用户确认时展示 displayChanges',
      },
      execute_payload: {
        type: 'object',
