...
CREATE TABLE IF NOT EXISTS pms_project_plan_item (
  ...
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  CONSTRAINT ck_project_plan_item_status CHECK (status IN (0,1,2,3)),
  CONSTRAINT ck_project_plan_item_previous_status CHECK (previous_status IS NULL OR previous_status IN (0,1)),
  CHECK (requires_delivery_file = 0 OR NULLIF(BTRIM(delivery_requirement), '') IS NOT NULL),
  CHECK (status = 2 OR actual_end_date IS NULL),
  CHECK (status <> 3 OR NULLIF(BTRIM(pause_reason), '') IS NOT NULL)
);
...
CREATE TABLE IF NOT EXISTS pms_task (
  ...
  status SMALLINT NOT NULL DEFAULT 0 CHECK (status IN (0,1,2,3)),
  is_overdue SMALLINT NOT NULL DEFAULT 0 CHECK (is_overdue IN (0,1)),
  start_date DATE, expected_end_date DATE, actual_end_date DATE, suspend_date DATE, suspend_reason VARCHAR(200),
  creator_id BIGINT REFERENCES pms_user(id) ON DELETE SET NULL,
  updater_id BIGINT REFERENCES pms_user(id) ON DELETE SET NULL,
  is_deleted SMALLINT NOT NULL DEFAULT 0 CHECK (is_deleted IN (0,1)),
...
CREATE TABLE IF NOT EXISTS pms_open_client (
  ...
  created_by BIGINT REFERENCES pms_user(id) ON DELETE SET NULL,
  updated_by BIGINT REFERENCES pms_user(id) ON DELETE SET NULL,
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
CREATE UNIQUE INDEX IF NOT EXISTS uk_product_maintenance_contract_code_active ON pms_product_maintenance_contract(contract_code) WHERE is_deleted = 0;
CREATE UNIQUE INDEX IF NOT EXISTS uk_product_maintenance_contract_root_active ON pms_product_maintenance_contract(product_id) WHERE previous_contract_id IS NULL AND is_deleted = 0;
CREATE UNIQUE INDEX IF NOT EXISTS uk_product_maintenance_contract_previous_active ON pms_product_maintenance_contract(previous_contract_id) WHERE previous_contract_id IS NOT NULL AND is_deleted = 0;
CREATE INDEX IF NOT EXISTS idx_product_maintenance_contract_product_active ON pms_product_maintenance_contract(product_id, service_start_date DESC) WHERE is_deleted = 0;
CREATE INDEX IF NOT EXISTS idx_product_maintenance_contract_supplier_active ON pms_product_maintenance_contract(supplier_id) WHERE is_deleted = 0;
CREATE INDEX IF NOT EXISTS idx_product_maintenance_contract_expiry_active ON pms_product_maintenance_contract(service_end_date) WHERE is_deleted = 0 AND termination_date IS NULL;
CREATE UNIQUE INDEX IF NOT EXISTS uk_product_maintenance_contract_attachment_storage_name ON pms_product_maintenance_contract_attachment(storage_name);
CREATE INDEX IF NOT EXISTS idx_product_maintenance_contract_attachment_contract_active ON pms_product_maintenance_contract_attachment(contract_id, sort_order) WHERE is_deleted = 0;
...
CREATE INDEX IF NOT EXISTS idx_mcp_client_active ON pms_mcp_client(endpoint_type, status, is_deleted, expires_at);
CREATE UNIQUE INDEX IF NOT EXISTS ux_mcp_audit_request ON pms_mcp_audit_log(request_id);
CREATE INDEX IF NOT EXISTS idx_mcp_audit_client_created ON pms_mcp_audit_log(client_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_mcp_audit_user_created ON pms_mcp_audit_log(user_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_mcp_audit_tool_created ON pms_mcp_audit_log(tool_name, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_mcp_ticket_status_expires ON pms_mcp_action_ticket(status, expires_at);
CREATE UNIQUE INDEX IF NOT EXISTS ux_mcp_ticket_idempotency ON pms_mcp_action_ticket(client_id, user_id, tool_name, idempotency_key) WHERE idempotency_key IS NOT NULL;
...
CREATE INDEX IF NOT EXISTS idx_project_plan_template_enabled ON pms_project_plan_template(status, sort_order, id);
...
ON CONFLICT(code) DO UPDATE SET name=EXCLUDED.name,description=EXCLUDED.description,status=EXCLUDED.status,sort_order=EXCLUDED.sort_order,updated_at=NOW();

INSERT INTO pms_project_plan_template_stage(template_id,name,description,sort_order)
SELECT t.id,v.name,NULL,v.sort_order FROM pms_project_plan_template t
CROSS JOIN (VALUES
  ('项目立项',0),('场景定义与前期筹备',1),('数据准备与环境搭建',2),('模型开发与技术落地',3),
  ('系统联调与功能验证',4),('试运行与优化完善',5),('项目验收与成果交付',6)
) v(name,sort_order)
WHERE t.code='ai_project_standard'
ON CONFLICT(template_id,name) DO UPDATE SET sort_order=EXCLUDED.sort_order,updated_at=NOW();

INSERT INTO pms_project_plan_template_item(template_stage_id,name,requires_delivery_file,delivery_requirement,remark,sort_order)
SELECT s.id,v.item_name,v.requires_delivery,CASE WHEN v.requires_delivery=1 THEN '关键交付文件' ELSE NULL END,NULL,v.item_sort
FROM pms_project_plan_template t
JOIN pms_project_plan_template_stage s ON s.template_id=t.id
JOIN (VALUES
  ('项目立项','需求场景说明',0,0),('项目立项','立项汇报',1,1),
  ('场景定义与前期筹备','项目团队组建',0,0),('场景定义与前期筹备','场景需求调研及评审',1,1),
  ('场景定义与前期筹备','项目主计划',0,2),('场景定义与前期筹备','需求说明书及评审',1,3),
  ('场景定义与前期筹备','实施方案及评审',1,4),('场景定义与前期筹备','外部供应商商务',0,5),
  ('场景定义与前期筹备','项目启动会',0,6),
  ('数据准备与环境搭建','原始数据准备',1,0),('数据准备与环境搭建','数据治理',0,1),
  ('数据准备与环境搭建','数据对接规范与接口开发',0,2),('数据准备与环境搭建','开发&测试环境搭建',0,3),
  ('模型开发与技术落地','方案设计与评审',1,0),('模型开发与技术落地','模型训练与调优',0,1),
  ('模型开发与技术落地','功能模块开发',0,2),('模型开发与技术落地','系统集成与联调',0,3),
  ('系统联调与功能验证','功能测试',0,0),('系统联调与功能验证','用户测试',0,1),
  ('系统联调与功能验证','测试报告',1,2),('系统联调与功能验证','上线确认',0,3),
  ('系统联调与功能验证','生产环境部署',0,4),
  ('试运行与优化完善','用户培训',0,0),('试运行与优化完善','系统运维',0,1),
  ('试运行与优化完善','试运行报告',1,2),
  ('项目验收与成果交付','成果准备',0,0),('项目验收与成果交付','项目验收',1,1)
